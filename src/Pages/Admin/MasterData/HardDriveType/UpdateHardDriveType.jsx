...
const UpdateHardDriveType = ({
  isVisible,
  setVisibility,
  refreshBrandData,
}) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("UpdateHardDriveType")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/HardDrive Type/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(
        `/brand/HardDriveType/${updateBrandData?._id}`,
        {
          name: name,
      ...
      toast.success(`HardDrive Type has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateHardDriveType;
