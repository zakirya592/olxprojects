...
const UpdateStorey = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateStorey"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/storey/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`storey has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
