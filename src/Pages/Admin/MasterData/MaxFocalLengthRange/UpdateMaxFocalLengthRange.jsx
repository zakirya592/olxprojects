...
const UpdateMaxFocalLengthRange = ({
  isVisible,
  setVisibility,
  refreshBrandData,
}) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updateMaxFocalLengthRange")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/MaxFocalLengthRange/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`MaxFocal Length Range has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateMaxFocalLengthRange;
