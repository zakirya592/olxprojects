...
const UpdateRegistrationCity = ({
  isVisible,
  setVisibility,
  refreshBrandData,
}) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updateRegistrationCity")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/RegistrationCity/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Registration City has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateRegistrationCity;
