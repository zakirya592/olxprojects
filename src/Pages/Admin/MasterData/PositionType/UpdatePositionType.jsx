...
const UpdatePositionType = ({
  ...
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updatePositionType")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/PositionType/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Position Type has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdatePositionType;
