...
const Wifi = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateWifi", JSON.stringify(row));
  };
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete("/brand/Wifi/" + row?._id);
          if (isDeleted) {
            toast.success(`Wifi has been deleted successfully!`);
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-4 bg-white shadow-xl rounded-md">
              <div className={`flex px-3 flex-row justify-start`}>
                ...
                >
                  <i className="fas fa-plus mr-2"></i>Add Wifi
                </button>
              </div>
