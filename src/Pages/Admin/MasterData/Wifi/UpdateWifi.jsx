...
const UpdateWifi = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updateWifi"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Wifi/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Wifi has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
