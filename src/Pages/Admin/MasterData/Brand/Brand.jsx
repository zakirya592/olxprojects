...
const Brand = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateBrand", JSON.stringify(row));
  };
...
//     setIsLoading(true);
//     try {
//       const response = await NewRequest.get("/brand");
...
const fetchData = async () => {
  ...
  try {
    ...
    const footerCategoryMap = footerCategories.reduce((acc, footerCategory) => {
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete("/brand/" + row?._id);
          if (isDeleted) {
