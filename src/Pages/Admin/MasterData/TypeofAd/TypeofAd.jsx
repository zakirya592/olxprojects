...
import AddTypeofAd from "./AddTypeofAd";
import UpdateTypeofAd from "./UpdateTypeofAd";
...
const TypeofAd = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateTypeofAd", JSON.stringify(row));
  };
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete(
            "/brand/TypeofAd/" + row?._id
          );
          if (isDeleted) {
            toast.success(`Type of Ad has been deleted successfully!`);
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-4 bg-white shadow-xl rounded-md">
              <div className={`flex px-3 flex-row justify-start`}>
                ...
                >
                  <i className="fas fa-plus mr-2"></i>Add Type of Ad
                </button>
              </div>
