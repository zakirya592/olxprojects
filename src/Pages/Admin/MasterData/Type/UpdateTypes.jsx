...
const UpdateTypes = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updateTypesection"));
  ...
  const footerCategorydata = async () => {
   ...
   const getiddata = async () => {
      try {
        const response = await NewRequest.get(`/brand/type/${updateBrandData._id}`);
        setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(
        `brand/type/${updateBrandData?._id}`,
        {
          name: name,
