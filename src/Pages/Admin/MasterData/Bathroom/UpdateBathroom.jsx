...
const UpdateBathroom = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updatebathroom"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/bathroom/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`BedRoom has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
