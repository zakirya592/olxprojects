...
const UpdateConstruction = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateConstruction"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Construction/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Construction has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
