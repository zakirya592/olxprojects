...
const UpdateLanguage = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updateLanguage")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Language/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Language has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
