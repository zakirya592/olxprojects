...
const UpdateResolution = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updateResolution"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Resolution/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Resolution has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
