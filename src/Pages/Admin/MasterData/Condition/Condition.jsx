...
const Condition = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateCondition", JSON.stringify(row));
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const [brandResponse, subCategoryResponse, footerCategoryResponse] =
        await Promise.all([
          NewRequest.get("/brand/condition"),
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete("/brand/condition/" + row?._id);
          if (isDeleted) {
