...
const UpdateAreaunit = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateAreaunit"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Area Unit/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(
        `/brand/areaunit/${updateBrandData?._id}`,
        {
          name: name,
      ...
      toast.success(`Area Unit has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
