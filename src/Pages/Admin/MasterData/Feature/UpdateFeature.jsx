...
const UpdateFeature = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateFeature"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Feature/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Feature has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
