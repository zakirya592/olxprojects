...
const UpdateBreed = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updateBreed")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Breed/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Breed has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
