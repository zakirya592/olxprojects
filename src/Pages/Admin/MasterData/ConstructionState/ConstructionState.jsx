...
import AddConstructionState from "./AddConstructionState";
import UpdateConstructionState from "./UpdateConstructionState";
...
const ConstructionState = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("UpdateConstructionState", JSON.stringify(row));
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const [brandResponse, subCategoryResponse, footerCategoryResponse] =
        await Promise.all([
          NewRequest.get("/brand/ConstructionState"),
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete(
            "/brand/ConstructionState/" + row?._id
          );
          if (isDeleted) {
            toast.success(`Construction State has been deleted successfully!`);
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-4 bg-white shadow-xl rounded-md">
              <div className={`flex px-3 flex-row justify-start`}>
                ...
                >
                  <i className="fas fa-plus mr-2"></i>Add Construction State
                </button>
              </div>
