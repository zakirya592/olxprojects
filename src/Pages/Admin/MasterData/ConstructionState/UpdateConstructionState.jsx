...
const UpdateConstructionState = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateConstructionState"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Construction State/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(
        `/brand/ConstructionState/${updateBrandData?._id}`,
        {
          name: name,
      ...
      toast.success(`Construction State has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateConstructionState;
