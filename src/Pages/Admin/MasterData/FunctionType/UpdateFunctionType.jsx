...
const UpdateFunctionType = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateFunctionType"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/FunctionType/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Function Type has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateFunctionType;
