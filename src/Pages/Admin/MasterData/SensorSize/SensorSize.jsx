...
import AddSensorSize from "./AddSensorSize";
import UpdateSensorSize from "./UpdateSensorSize";
...
const SensorSize = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateSensorSize", JSON.stringify(row));
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const [brandResponse, subCategoryResponse, footerCategoryResponse] =
        await Promise.all([
          NewRequest.get("/brand/SensorSize"),
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete(
            "/brand/SensorSize/" + row?._id
          );
          if (isDeleted) {
            toast.success(`Sensor Size has been deleted successfully!`);
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-4 bg-white shadow-xl rounded-md">
              <div className={`flex px-3 flex-row justify-start`}>
                ...
                >
                  <i className="fas fa-plus mr-2"></i>Add Sensor Size
                </button>
              </div>
