...
const UpdateSensorSize = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updateSensorSize"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/SensorSize/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Sensor Size has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateSensorSize;
