...
const UpdateHiringPerson = ({
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/HiringPerson/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`Hiring Person has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateHiringPerson;
