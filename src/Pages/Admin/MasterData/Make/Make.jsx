...
import { AdminMake } from "../../../../../utils/Datatablesource";
...
const Make = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updatemake", JSON.stringify(row));
  };
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete("/brand/make/" + row?._id);
          if (isDeleted) {
            toast.success(`Make has been deleted successfully!`);
