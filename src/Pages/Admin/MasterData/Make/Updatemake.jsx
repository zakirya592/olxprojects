...
const Updatemake = ({ isVisible, setVisibility, refreshBrandData }) => {
    ...
    const footerCategorydata = async () => {
     ...
     const getiddata = async () => {
      try {
        const response = await NewRequest.get(`/brand/make/${updateBrandData._id}`);
        setsubCategory({
    ...
    const handleAddCompany = async () => {
        try {
            ...
            const response = await NewRequest.put(
                `brand/make/${updateBrandData?._id}`, formData
            );
            toast.success(`Make has been update successfully".`, {
                position: "top-right",
                autoClose: 2000,
