...
const UpdateEngineCapacity = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(
    sessionStorage.getItem("updateEngineCapacity")
  );
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/EngineCapacity/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      ...
      toast.success(`EngineCapacity has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateEngineCapacity;
