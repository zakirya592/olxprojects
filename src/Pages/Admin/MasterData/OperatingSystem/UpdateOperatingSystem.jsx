...
const UpdateOperatingSystem = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("UpdateOperatingSystem"));
  ...
  const getiddata = async () => {
    try {
      const response = await NewRequest.get(
        `/brand/Operating System/${updateBrandData._id}`
      );
      setsubCategory({
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(
        `/brand/OperatingSystem/${updateBrandData?._id}`,
        {
          name: name,
      ...
      toast.success(`Operating System has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
...
export default UpdateOperatingSystem;
